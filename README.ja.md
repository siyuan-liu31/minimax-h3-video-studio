...
- トップレベルの「復刻ワークショップ」は 固定の総尺上限を設けない元動画、設定、保持項目、置換対象、任意の画像参照を受け取ります。シーンを解析し、15.08 秒以下の H3 セグメントに変換し、結合後に元の正確な長さまでトリムします。短い動画はモデル入力だけを補完します。アップロード容量、プロジェクト JSON 容量、ストレージにはサーバー設定の制約があります。
