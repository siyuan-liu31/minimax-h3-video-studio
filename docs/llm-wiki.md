...
- 前端复刻工坊分为素材与方案、分段审阅、成片；“复刻说明”复用画布的 `@` 图片资产选择器，插入稳定 `@{asset-id}` 标签时同步勾选替换参考，规划前检查标签确实已连接。保存草稿和启动生成独立。服务端列表 + localStorage 选中 ID 恢复当前项目；分段分页、来源/结果对照、修改范围提示、未保存修改保护、保存时原始版本乐观锁、轮询恢复。跳转长视频携带 project ID；`serializeVideoProject` 必须保留 recipe，避免丢失精确裁帧/音频策略。详细设计见 `docs/replication-workflow.md`。
